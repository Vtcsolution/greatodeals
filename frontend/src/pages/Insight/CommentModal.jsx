...
import { X, Send, User, Calendar, MessageCircle, ThumbsUp, Loader2, AlertCircle, Reply, ChevronRight } from 'lucide-react';
...
const CommentModal = ({ blogId, isOpen, onClose, onCommentAdded }) => {
  ...
  const renderComments = (comments, level = 0) => (
    <div className={`space-y-4 ${level > 0 ? 'ml-8 pl-4 border-l-2 border-gray-300' : ''}`}>
      {comments.map((comment) => (
        <div key={comment._id} className={`bg-white rounded-lg p-4 border border-gray-200 ${level > 0 ? 'bg-gray-50' : ''}`}>
          <div className="flex items-start justify-between mb-2">
            <div className="flex items-center gap-3 flex-1">
              <div className={`w-8 h-8 bg-gradient-to-br ${getAuthorAvatar(comment.username)} rounded-full flex items-center justify-center text-white font-bold text-xs flex-shrink-0`}>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-1 px-2 py-1 text-xs bg-gray-100 hover:bg-red-50 text-gray-600 hover:text-red-600 rounded transition"
                aria-label={`Like ${comment.username}'s comment (${comment.likes || 0} likes)`}
              >
                <ThumbsUp className="w-3 h-3" fill={comment.likes > 0 ? 'currentColor' : 'none'} />
                <span>{comment.likes || 0}</span>
              </button>
              <button
                onClick={() => handleReply(comment)}
                className="flex items-center gap-1 px-2 py-1 text-xs bg-gray-100 hover:bg-blue-50 text-gray-600 hover:text-blue-600 rounded transition"
          ...
          <p className="text-gray-700 text-sm leading-relaxed ml-11">{comment.comment}</p>
          {comment.replies && comment.replies.length > 0 && (
            <div className="mt-3 ml-11 text-xs text-gray-500 flex items-center gap-1">
