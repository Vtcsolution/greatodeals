...
const Navbar = () => {
  ...
  return (
    <>
      ...
      <motion.button
        onClick={scrollToTop}
        className={`fixed bottom-4 right-4 p-3 bg-teal-500 hover:bg-teal-600 rounded-full transition-all duration-300 z-50 ${
      ...
      {!isChatOpen && (
        <motion.div
          className="fixed bottom-16 right-4 sm:right-6 bg-[#2E7D7B] p-3 rounded-full cursor-pointer shadow-lg hover:bg-[#266966] transition-all duration-300 z-50 flex items-center gap-2 pr-4 group"
          onClick={() => setIsChatOpen(true)}
          whileHover={{ scale: 1.1 }}
          whileTap={{ scale: 0.9 }}
        >
          <MessageCircle className="w-6 h-6 text-[#F5F6F5]" />
          <span className="text-sm font-semibold text-[#F5F6F5] ">AI Greato</span>
        </motion.div>
      )}
      {/* Chat Section */}
      <AnimatePresence>
