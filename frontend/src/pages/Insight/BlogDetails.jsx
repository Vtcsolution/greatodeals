...
import {
  ...
  Facebook, Twitter, Linkedin, MessageCircle, Heart, Tag,
...
const BlogDetails = () => {
  ...
  const handleLikeComment = async (commentId) => {
    try {
      await axios.put(`${import.meta.env.VITE_BASE_URL}/api/comments/${commentId}/like`);
      fetchComments();
    } catch (err) {
  ...
  const handleLikeBlog = () => setIsLiked(!isLiked);
  ...
  const renderCommentThread = (comment, level = 0) => {
    ...
    const indentClass = level > 0 ? 'ml-0 sm:ml-8 pl-0 sm:pl-4 border-l-0 sm:border-l-2 border-gray-200' : '';
    ...
    return (
      ...
      >
        <div className="flex items-start justify-between mb-3">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 bg-gray-50 text-black hover:text-red-600 hover:bg-red-50 rounded-lg transition-all text-xs sm:text-sm font-medium"
              aria-label={`Like ${comment.username}'s comment`}
            >
              <Heart
                className={`w-3.5 h-3.5 sm:w-4 sm:h-4 transition-all ${
                  comment.likes > 0 ? 'fill-red-600 text-red-600 scale-110' : ''
                }`}
              />
        ...
        <p className="text-black leading-relaxed mb-2 ml-0 sm:ml-13">{comment.comment}</p>
          ...
          <main className="lg:col-span-3 space-y-8">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6 mt-12 p-5 sm:p-6 bg-white rounded-2xl border shadow-sm">
              ...
              <button
                ...
                className="flex items-center gap-3 px-5 sm:px-6 py-2.5 sm:py-3 bg-gradient-to-r from-teal-600 to-blue-600 hover:from-teal-700 hover:to-blue-700 text-white font-medium rounded-xl transition-all shadow-lg hover:shadow-xl text-sm sm:text-base"
              >
            ...
            <section className="mt-12 sm:mt-16 text-center p-8 sm:p-10 bg-gradient-to-r from-teal-50 via-blue-50 to-indigo-50 rounded-3xl border border-teal-100 shadow-lg">
              ...
              <button
                onClick={() => setIsCommentModalOpen(true)}
                className="px-6 sm:px-8 py-3 bg-gradient-to-r from-teal-600 to-blue-600 hover:from-teal-700 hover:to-blue-700 text-white font-medium rounded-xl transition-all shadow-lg hover:shadow-xl text-sm sm:text-base"
              >
                Write a Comment
              </button>
            </section>
          ...
          <aside className="lg:col-span-1 lg:sticky lg:top-24 h-fit space-y-6 sm:space-y-8">
            ...
            <div className="bg-white p-5 sm:p-6 rounded-2xl shadow-sm border">
              <div className="text-center">
                ...
                <button className="w-full py-2.5 bg-gradient-to-r from-teal-600 to-blue-600 text-white font-medium rounded-xl hover:from-teal-700 hover:to-blue-700 transition-all shadow-sm text-sm">
                  Follow Author
                </button>
              </div>
            </div>
